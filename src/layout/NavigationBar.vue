<template>
  ...
  <v-toolbar color="primary" app>
    ...
    <v-spacer />
    <v-toolbar-items>
      <v-btn flat @click.prevent="logout" v-if="isLoggedIn">{{ __("buttons.logout") }}
      </v-btn>
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      ...mapComputed(store, 'isLoggedIn', session.types.getters.isLoggedIn, session.types.mutations.isLoggedIn)
    },
    methods: {
      async logout() {
        const { Auth } = this.Amplify
        return Auth.signOut()
          .then(() => this.$store.commit(session.types.mutations.isLoggedIn, false))
      ...
      toggleLang() {
        ...
        const nextIndex = currentIndex + 1 < languages.length ? currentIndex + 1 : 0
